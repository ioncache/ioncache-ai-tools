...
- **`docs_first_guard.py`**: requires a genuine documentation lookup before other tool use on flagged turns. Prevents acting on remembered API/CLI/library behavior that may be stale or simply wrong by the time it's used, training data has a cutoff and third-party interfaces change after it.
...
- **Never push without asking, every time.** Ask before every single push, no exceptions. Prior or implied permission never carries over; one approval covers exactly one push.
