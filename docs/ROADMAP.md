...
- **`git push` agent hook** (`hooks/hooks.json`, not a `hooks/rules/*` rule): denies `git push` unless the current turn explicitly and unambiguously authorized this specific push. A deterministic tokenizing rule (matching `never_kill_without_asking`'s approach) was tried first, but "did the user explicitly authorize this" is a judgment call over open-ended natural language, not something an exact-phrase or keyword match can cover without either missing real phrasing or accepting a loose match that risks a false authorization. Uses Claude Code's experimental `type: "agent"` PreToolUse hook instead: a subagent reads the transcript and judges the current turn's messages, denying on any doubt or ambiguity. Prior or implied permission never carries over, only the current turn's own message can authorize a given push. The `if: "Bash(*git*push*)"` pre-filter that decides whether to spawn the subagent at all matches raw command text, not the actual invoked program, so a command that just mentions "git" and "push" as substrings (e.g. an unrelated `echo` or `grep`, or a branch name containing "push") also triggers it; verified against real commands, including `git -C <dir> push` and `git --no-pager push`, which a naive `Bash(git push *)` filter misses entirely (see README's `git push` agent hook section).
- **`git commit` agent hook** (`hooks/hooks.json`, not a `hooks/rules/*` rule): same mechanism as the `git push` agent hook above, applied to `git commit`. Denies unless the current turn explicitly asked for a commit right now; "finish it"/"proceed", or a prior turn's "commit" instruction, does not carry forward to a later, unrelated commit. Uses `if: "Bash(*git* commit*)"` (a leading space before "commit", unlike push's pattern), which also avoids push's branch-name false positive for a branch whose name contains "commit" with a hyphen rather than a space before it. Being an LLM judgment rather than a script, it can occasionally misjudge in the opposite direction from the false positives documented for `git push`: in testing it once denied a commit that actually had been authorized earlier in the same turn, after several unrelated tool calls came between the authorization and the attempt; retrying the identical commit succeeded (see README's `git commit` agent hook section).
