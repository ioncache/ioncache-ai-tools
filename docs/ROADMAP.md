...
- **`never_push_without_asking`**: denies `git push` unless the user has explicitly asked for it, every single time. Prior or implied permission never carries over; a push approved earlier in the session (or earlier in the same turn) doesn't authorize the next one. Uses the same command-tokenization approach as `never_kill_without_asking`, extended to find `push` past `git`'s own global options (`git -C <dir> push`, `git --no-pager push`) rather than assuming it's always the very next token.
